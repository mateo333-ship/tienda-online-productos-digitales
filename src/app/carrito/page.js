  ...
  async function handleCheckout() {
    ...
    try {
      await withLoading(async () => {
        ...
        if (data.mode === "stripe") {
          // Salimos de la web hacia la página de pago de Stripe, donde se
          // piden el nombre, el email y el teléfono de entrega junto al
          // código de descuento. El overlay de carga se queda visible
          // hasta ese momento, así que no hace falta "apagarlo" a mano.
      ...
      {!user ? (
        ...
        <div className="mt-8 space-y-4">
          ...
          <p className="text-xs text-[var(--ink-soft)]">
            Un único pago seguro cubre todos los productos del carrito, sin cobros por separado. En
            la propia pantalla de pago te pediremos tu nombre, email y teléfono para poder
            entregarte la compra al instante.
          </p>
        </div>
      )}
    </div>
